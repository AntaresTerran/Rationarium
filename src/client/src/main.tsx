...
function aggregate(islands: Island[]): Aggregate[] {
  ...
  for (const island of islands) for (const item of island.entries) {
    ...
    if (item.stock !== undefined) row.stock = (row.stock || 0) + item.stock;
    row.islandKeys.push(island.key);
    rows.set(item.productGuid, row);
  }
  return [...rows.values()].map((row) => ({ ...row, averageProductivity: row.amountOfBuildings ? row.averageProductivity / row.amountOfBuildings : 0 }));
}
